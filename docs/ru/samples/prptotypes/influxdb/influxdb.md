...
Прототип представляет собой коннектор (шлюз) между вашей внутренней базой данных и базой данных временных рядов InfluxDB.

Основные возможности прототипа:
Автоматизированный сбор данных (Aggregation): Прототип умеет «пробегаться» по дереву объектов (используя Root и Patterns) и находить все активные датчики. Вам не нужно прописывать каждый датчик вручную — достаточно указать шаблон пути.

Преобразование форматов (Data Transformation): Код берет данные из внутреннего формата системы и «переводит» их на язык InfluxDB Line Protocol.

Measurement: Имя измерения (например, «Давление» или «Температура»).

Tags: Метаданные (например, ID цеха, модель станка), которые позволяют быстро фильтровать данные.

Fields: Реальные значения датчиков (числа или строки).

Группировка по объектам: Логика настроена так, что для одного базового объекта (например, одной насосной станции) формируется одна компактная строка, содержащая значения сразу всех её внутренних датчиков. Это значительно снижает нагрузку на сеть и базу данных.

Отказоустойчивость (Error Handling): В коде реализована защита: если один датчик выдает ошибку или имеет неверный формат тегов (JSON), вся система не «падает». Ошибка логируется, а сбор данных продолжается для остальных объектов.
